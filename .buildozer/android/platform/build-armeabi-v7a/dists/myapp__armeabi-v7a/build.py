...
def make_package(args):
    # If no launcher is specified, require a main_old.py/main.pyo:
    if (get_bootstrap_name() != "sdl" or args.launcher is None) and \
            get_bootstrap_name() not in ["webview", "service_library"]:
        # (webview doesn't need an entrypoint, apparently)
        if args.private is None or (
                not exists(join(realpath(args.private), 'main_old.py')) and
                not exists(join(realpath(args.private), 'main.pyo'))):
            print('''BUILD FAILURE: No main_old.py(o) found in your app directory. This
file must exist to act as the entry point for you app. If your app is
started by a file with a different name, rename it to main_old.py or add a
main_old.py that loads it.''')
            sys.exit(1)
    ...
    try:
        if args.private:
            if not use_setup_py or (
                ...
                print("Copying main_old.py's ONLY, since other app data is "
                      "expected in site-packages.")
                main_py_only_dir = tempfile.mkdtemp()
                _temp_dirs_to_clean.append(main_py_only_dir)

                # Check all main_old.py files we need to copy:
                copy_paths = ["main_old.py", join("service", "main_old.py")]
                ...
                # Append directory with all main_old.py's to result apk paths:
    ...
    service = False
    if args.private:
        service_main = join(realpath(args.private), 'service', 'main_old.py')
...
''')
    ...
    ap.add_argument('--private', dest='private',
                    help='the directory with the app source code files' +
                         ' (containing your main_old.py entrypoint)',
